...
gl = 0
import os
while os.path.exists(os.path.join(losspath,"hogvgg%d.txt"%gl)):
    gl+=1
lpath = os.path.join(losspath,"hogvgg%d.txt"%gl)
lf = open(lpath,'w')
...
def train(epoch):
    ...
    for batchidx, (data, target) in enumerate(dataloader):
        ...
        hogs = [ft.hog(j.reshape(3, 44, 44).transpose(1,2,0)) for j in tmpdata]
        hogs = np.array(hogs)
        hogs = torch.FloatTensor(hogs)
        if use_cuda:
            data,target = data.cuda(), target.cuda()
            hogs = hogs.cuda()
        data,target = Variable(data),Variable(target)
        optimizer.zero_grad()
        output = model(data, hogs)
...
def validate(best_acc,save = False):
    ...
    if(save):
        ...
        torch.save(model.state_dict(), "./models/hogvgg2model%.4f.pth"%acc)
        print("Saving in "+"./models/hogvgg2model%.4f.pth"%acc)
